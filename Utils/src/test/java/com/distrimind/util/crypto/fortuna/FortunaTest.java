...
public class FortunaTest {
    ...
    public void shouldProduceEvenDistribution() {
        ...
        assertEquals(0.0, percentDifferenceFortuna, 0.01, "UniformRealDistribution vs Fortuna variance");
    }
    ...
    public void shouldNotShutdownPassedInScheduledExecutorService() throws InterruptedException {
        ...
        try {
            ...
            fortuna.shutdown();
            for (ScheduledFuture<?> f : scheduler.getCreatedFutures()) {
                    assertTrue(f.isCancelled(), "Future was not cancelled");
            }
            assertFalse(scheduler.isShutdown(), "Scheduler was shut down");
        } finally {
            delegate.shutdown();
        }
    }
